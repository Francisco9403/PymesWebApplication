...
import TaxReports from "./TaxReports"; // <--- Importamos el nuevo componente
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 text-sm font-medium">Control de divisas y automatización de precios en tiempo real.</p>
            </div>
          </header>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {currencies.map((currency) => (
                <div key={currency.name} className="card-container p-6 space-y-2 group bg-white border-slate-200">
                  ...
                  <p className={`text-4xl font-black tracking-tighter ${currency.color}`}>
                    <span className="text-lg mr-1">$</span>{Number(currency.value).toFixed(2)}
                  </p>
            ...
            <MarkupConfigurator initialEnabled={config.enabled} initialThreshold={config.threshold} />
          </div>
