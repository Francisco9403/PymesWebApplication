    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                <div>
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">Mis Proveedores</h2>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Gestión comercial de Junín</p>
                </div>
                <button
                    onClick={onNewImport}
            ...
            <div className="card-container overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tr>
                            <th className="p-5 font-black text-slate-400 uppercase text-[10px] tracking-widest">Razón Social</th>
                            <th className="p-5 font-black text-slate-400 uppercase text-[10px] tracking-widest">CUIT / ID</th>
                            <th className="p-4 font-black text-slate-400 uppercase text-[10px] tracking-widest text-right">Saldo CC</th>
                        </tr>
                        </thead>
                        ...
                        {suppliers.map((s) => (
                            <tr
                                key={s.id}
                                onClick={() => onSupplierClick(s)} // 👈 Disparamos el click
                      ...
                      <span className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">
                        ...
                        {s.paymentMethod?.replace("_", " ") || "RESPONSABLE INSCRIPTO"}
                      </span>
                                    </div>
                                </td>
                                <td className="p-5">
