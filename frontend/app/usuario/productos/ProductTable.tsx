...
export default function ProductTable({
  ...
  const handleConfirmPrices = async () => {
    if (!confirm("¿Deseas aplicar todos los aumentos sugeridos por la IA? Los productos protegidos no serán afectados.")) return;
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert(`¡Análisis completado! Se generaron ${result.count} sugerencias estratégicas.`);
      ...
      <div className="space-y-6 animate-in fade-in duration-700">
        ...
        <div className="flex flex-col md:flex-row justify-end items-center gap-3 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="mr-auto">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Estrategia de Precios</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Asistente Gemini 2.0 Flash</p>
          </div>
        ...
        <div className="card-container">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              {content.map((product) => {
                const hasStrategicRule = product.strategicMultiplier && Number(product.strategicMultiplier) > 1;
                ...
                return (
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        ...
                        <span className={`font-mono font-black px-2 py-1 rounded-lg transition-colors ${
                            hasStrategicRule
