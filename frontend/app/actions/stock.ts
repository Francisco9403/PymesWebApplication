...
import { Branch, ProductStock } from "@/types/ProductStock";
...
export async function getStockByBranch(branchId: number): Promise<ProductStock[]> {
    const cookieStore = await cookies();
    const jwt = cookieStore.get("token")?.value;
    if (!jwt) return [];

    const res = await fetch(`${process.env.NEXT_PUBLIC_API}/stock/branch/${branchId}`, {
        cache: "no-store",
        headers: { Authorization: `Bearer ${jwt}` },
    });
